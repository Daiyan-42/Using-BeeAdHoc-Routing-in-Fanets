...
public:
    ...
    bool        GetHasPiggyback() const { return m_hasPiggyback != 0; }
    uint32_t    GetPiggybackLen() const { return m_piggybackLen; }

    // Route accumulation
    void AddHop(Ipv4Address addr);
    void DecrementTtl() { if (m_ttl > 0) m_ttl--; }
    bool TtlExpired()   const { return m_ttl == 0; }
    void SetHasPiggyback(bool v) { m_hasPiggyback = v ? 1 : 0; }
    void SetPiggybackLen(uint32_t l) { m_piggybackLen = l; }
...
private:
    ...
    uint8_t                  m_hasPiggyback {0};
    uint32_t                 m_piggybackLen {0};
};
...
public:
    ...
    bool PeekForager(Ipv4Address dst, ForagerEntry& out) const;
...
private:
    ...
    void SendBackwardScout(const ForwardScoutHeader& fsh, Ptr<Packet> payload);
    ...
    void LaunchForwardScoutWithData(Ipv4Address dst,
                                    ForagerType type,
                                    Ptr<const Packet> dataPacket,
                                    const Ipv4Header& dataHdr);
    bool SendForagerWithData(const ForagerEntry& fe,
                             Ptr<const Packet> dataPacket,
                             const Ipv4Header& dataHdr);
    bool ExtractPiggybackPayload(Ptr<Packet> pkt,
                                 uint32_t payloadLen,
                                 Ptr<Packet>& outPacket,
                                 Ipv4Header& outHdr);
    void DeliverPiggybackData(Ptr<Packet> dataPacket,
                              const Ipv4Header& dataHdr);
    void DeliverToSource(Ptr<Packet> dataPacket,
                         const Ipv4Header& dataHdr);
    void RemoveFromPackerQueue(Ipv4Address dst,
                               const Ipv4Header& dataHdr);
    ...
    Time     m_scoutRetryInterval {Seconds(5.0)};
    Time     m_packerCheckInterval {Seconds(1.0)};
